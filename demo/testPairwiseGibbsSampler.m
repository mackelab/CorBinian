...
nSamples = 1000; burnIn = 500; 
thinning = 1;
model = 'ising_count_l_0';
mode = 'default'; % = 'RB';
output = 'samples';
sampleMode = 'pair'; % = 'pair';
newLambda = true;
nRestarts = 3;
...
switch sampleMode
  case 'single'
   [xSampled] = maxEnt_gibbs(nSamples, burnIn, thinning, lambda, x0, model,mode);
  case 'pair'
   [xSampled] = maxEnt_gibbs_pair(nSamples, burnIn, thinning, lambda, x0, model, mode, output);
end
switch output
  case 'samples'
  ...
  % Update figure:
   hold on
   plot( mean(fxSampled(end/2:end,:),1)', 'go-', 'linewidth', 1)
...
if d < 20
 ...
 switch output
  ...
  case 'means'
   legend('true', 'sampled discrete', 'MCMC chain', ...
          'location', 'north')         
 end
else
 ...
 switch output
  ...
  case 'means'
   legend('MCMC chain', 'location', 'north')
 end                         
end
hold off
